...
export type GetResponse = Document | {
    error: Status;
};
...
export type CustomDocument<T extends Record<string, any>> = Pick<Document, 'createTime' | 'updateTime'> & {
    id: string;
} & {
    fields: T;
};
...
type FieldFilterOp = 'OPERATOR_UNSPECIFIED' | 'LESS_THAN' | 'LESS_THAN_OR_EQUAL' | 'GREATER_THAN' | 'GREATER_THAN_OR_EQUAL' | 'EQUAL' | 'NOT_EQUAL' | 'ARRAY_CONTAINS' | 'IN' | 'ARRAY_CONTAINS_ANY' | 'NOT_IN';
